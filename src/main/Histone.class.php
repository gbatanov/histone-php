...
class Histone {
	...
	private static function processAddition($left, $right, $stack) {
		...
		$left = HistoneType::toString($left);
		$right = HistoneType::toString($right);
		return ($left . $right);
	}
	...
	public static function processCall($target, $name, $args, $stack) {
		...
		if ($target === null) {
			...
			if (method_exists('HistoneGlobal', $name) ||
				method_exists('HistoneGlobal', $name . '_internal') ||
